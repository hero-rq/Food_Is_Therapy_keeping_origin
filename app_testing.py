...
# In-memory storage for user habits (for demo purposes)
user_habits = {}
...
def generate_recipe():
    ...
    instructions_text = clean_text(instructions_text)  # Clean the text
...
def generate_fortunes(user_input):
    prompt = (
        f"Please write a fortune according to the {user_input}"
        f"you are like a tarot magician or fortune teller"
        "you are very creative to tell fortunes according to users taste"
        "your answers should be about 100 words, at most 120 words and please add emojis"
    )
    try:
        response = openai.ChatCompletion.create(
            model='gpt-4o-mini',
